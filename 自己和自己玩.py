...
sqlcount = "select symbol from(select symbol,count(close_price) as cc from day_price_alpha group by symbol ) A where cc >252 "
countt= pd.read_sql(sql=sqlcount, con=conn)
j=0
for row in countt.iterrows():
     sqlx = "select close_price,good_awareness  from day_price_alpha where symbol='%s' ORDER BY price_date desc  limit 252" %row[1]['symbol']
     sqlshindex = "select close_price  from shindex  order by datep desc limit 252"
     dfx = pd.read_sql(sql=sqlx, con=conn)
     #cha = dfx.head(1)['close_price'] - dfx.tail(1)['close_price']
     j=j+1
     # a = preprocessing.minmax_scale(dfx['close_price'].iloc[0], feature_range = (-1, 1))
     # b = preprocessing.minmax_scale(dfx['close_price'].iloc[-1], feature_range = (-1, 1))
     x_data = preprocessing.minmax_scale(dfx, feature_range=(-1, 1))
     # print(x_data[251:252, 0:1])
     # print(x_data[0:1, 0:1])
     dfy = pd.read_sql(sql=sqlshindex, con=conn)
     y_data = preprocessing.minmax_scale(dfy, feature_range=(-1, 1))
     if j == 10 :
         break
     for i in range(100):
           ...
           if i % 100 == 0:
               hello = loss_emotion[0]-loss_compare[0]
               File.write(str(row[1]['symbol']))
               File.write(str(loss_emotion))
               File.write(str(loss_compare))
               File.write(str(hello)+" ")
               File.write(str(dfx['close_price'].iloc[0])+" ")
               File.write(str(dfx['close_price'].iloc[-1])+" ")
               File.write(str(x_data[251:252, 0:1]-x_data[0:1, 0:1]) + "\n")
