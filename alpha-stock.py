...
lr=0.000618        #学习率
#——————————————————导入数据——————————————————————
f=open('stock20170331-0721.csv')
df=pd.read_csv(f)     #读入股票数据
data=df.iloc[:,0:6].values  #取第3-10列


#获取训练集
def get_train_data(batch_size=62,time_step=20,train_begin=0,train_end=324692):
    batch_index=[]
...
#获取测试集
def get_test_data(time_step=20,test_begin=324692):
...
#——————————————————训练模型——————————————————
def train_lstm(batch_size=62,time_step=20,train_begin=0,train_end=324692):
   ...
   #module_file = tf.train.latest_checkpoint()
    with tf.Session() as sess:
        ...
        base_path = saver.save(sess, "module/TQ120-reveune.model")
...
def prediction(time_step=20):
    ...
    with tf.Session() as sess:
        #参数恢复
        #module_file = tf.train.latest_checkpoint()
        saver.restore(sess, "module/TQ120-reveune.model")
        test_predict=[]
